...
from utils import ensure_dir, save_bytes, read_text_file
...
async def start(message):
    await message.answer(
        "Привет! Я RAG-бот.\n\n"
        "1) Пришли мне файл .txt или .md\n"
        "2) Потом задай интересующий вопрос по файлу.\n"
        "Команды:\n"
...
# 1) Telegram → JSON (update)
# 2) aiogram → Message (Python-объект)
# 3) message.document → Document (метаданные файла)
# 4) document.file_id → идентификатор
# 5) bot.get_file(file_id) → File (метаданные + file_path)
# 6) bot.download_file(file_path) → поток байтов
# 7) .read() → bytes